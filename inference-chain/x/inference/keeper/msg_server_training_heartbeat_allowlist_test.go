...
func TestTrainingHeartbeat_AllowListEnforced(t *testing.T) {
	k, ctx := keepertest.InferenceKeeper(t)
	ms := keeper.NewMsgServerImpl(k)
	wctx := sdk.UnwrapSDKContext(ctx)

	creator := "gonka1hgt9lxxxwpsnc3yn2nheqqy9a8vlcjwvgzpve2"
	req := &types.HeartbeatRequest{NodeId: creator + "/node1", RunId: 1, OuterStep: 0}

	// not allowed
	_, err := ms.TrainingHeartbeat(wctx, &types.MsgTrainingHeartbeat{Creator: creator, Req: req})
	require.Error(t, err)
